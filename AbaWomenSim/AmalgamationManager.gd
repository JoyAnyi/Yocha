...
@onready var summary_character = $SummaryCharacter
...
# Fade out effect
	...
	if index >= timeline.size() or index == -1: # -1 is a special code to force the end
		...
		# Start the dialogue tree at the intro node
		play_dialogue_node("amalgamation_intro")
		
		# ANIMATION PHASE 1: The Walk
		# Play the walking animation before the movement starts
		summary_character.play("walk")
		
		# Create the tween to slide him on screen
		var end_tween = create_tween().set_parallel(true)
		end_tween.tween_property(self, "modulate:a", 1.0, 0.5)
		
		# Slides from -400 to 100 over 1.5 seconds.
		end_tween.tween_property(summary_character, "position:x", 600, 1.5).set_trans(Tween.TRANS_SINE)
...
func _on_restart_pressed():
	summary_character.position.x = -950
	player.money = 100000
...
func play_dialogue_node(node_name: String):
	if node_name == "restart":
		_on_restart_pressed()
		return
	if node_name == "menu":
		_on_exit_to_menu_button_pressed()
		return
		
	var current_node = dialogue_data[node_name]
	
	# Update text
	title_label.text = "Epilogue"
		# Grab the raw text from the JSON
	var raw_text = current_node["text"]
	
	# Create a dictionary of the live stats to inject
	var live_stats = {
		"money": str(player.money),
		"risk": str(player.risk)
	}
	
	# Format the text and push it to the UI
	desc_label.text = raw_text.format(live_stats)

	
	# Update buttons
	btn_a.text = current_node["btn_a_text"]
	...
	btn_b.visible = false
	frame.visible = false
	# Disconnect old signals so the buttons don't trigger trades anymore
	if btn_a.pressed.is_connected(_on_button_a_pressed):
		btn_a.pressed.disconnect(_on_button_a_pressed)
		#btn_b.pressed.disconnect(_on_button_b_pressed)
		
	# Connect temporary signals that pass the next target to this function
	# Note: bind is used to pass the target string from the JSON
	btn_a.pressed.connect(play_dialogue_node.bind(current_node["btn_a_target"]), CONNECT_ONE_SHOT)
	#btn_b.pressed.connect(play_dialogue_node.bind(current_node["btn_b_target"]), CONNECT_ONE_SHOT)


func handle_political_event(choice: String):
	...
	if choice == "A":
		...
		influence_change = current_event_data.influence_change_b
		stability_change = current_event_data.stability_change_b

	player.update_influence(influence_change)
	player.update_stability(stability_change)

	feedback_label.text = "Political decision recorded."
